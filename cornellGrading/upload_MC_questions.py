...
courseNumbers = [re.findall('\((.+?)\)', i)[-1] for i in courseNames] # pull out rightmost instance of a string in between parentheses
...
courseChosen=easygui.choicebox('Select canvas course (or exit)','Course Selector (or exit)',choices=courseNames)
...
# Convert input latex string to Canvas's img html
    ...
    if isinstance(texstr,re.Match): texstr = texstr.groups()[0] 
    ...
    qtxt = """<img class="equation_image" title="{0}" src="https://canvas.cornell.edu/equation_images/{1}" alt="LaTeX: {0}">""".format(texstr,urllib.parse.quote(urllib.parse.quote(texstr)))
...
filename=easygui.fileopenbox(msg='Select a txt multiple-choice question bank input file',title='Input File Selector',default='./*_MC.txt')#,'txt multiple-choice source files'
data = np.genfromtxt(filename,delimiter='\t',dtype=str)  #numpy function to load data from text
...
q = c.course.create_quiz({'title':'dummy quiz'}) # creates quiz in course; title is 2nd string
...
for row in data: # loop through rows in data
    ...
    for a in np.reshape(row[2:],(int(row[2:].size/3),3)):  #takes cols in pairs starting with third one and puts in a
        answers.append({'answer_html':convall(a[0])}) # assign answer text.  put in answer_html not answer_text so that latex can be processed
        if a[1]=='correct': # parsing text file for the blackboard input standard and switching to the canvas input standard
        ...
        answers.append({'comments_html':convall(a[2])}) # comments in response to right or wrong answers 
    ...
    q.create_question(question={'question_text':convall(row[1]),\
            'question_type':'multiple_choice_question','answers':answers})   # in canvas, upload question
